...
**指令全部保留；同时支持自然语言触发**：对「非 / 开头」的消息做意图匹配，命中则复用对应功能并终止后续 LLM，避免重复回复。自然语言可在配置中按模块开关。

---

### 功能与指令规划（指令保留，自然语言为增量）

- **火车票模块**  
  - 主命令前缀：`/火车票`（或沿用原有别名，迁移时统一整理）。  
  - 自然语言：如「从厦门到上海的车票」「北京到广州的火车票」；**出发地、目的地均需命中中国城市**（见 `train_cities.py` 城市集合），避免误触发（如「从家到上海」「北京到公司」不触发）。
...
- **天气模块**  
  ...
  - 自然语言：如「北京天气」「上海今天多少度」等，由 **natural_language** 配置统一开关。
...
- **自然语言触发（统一）**  
  - 对**非 / 开头**的消息尝试意图匹配；命中则调用对应模块逻辑并 **event.stop_event()**，不再走 LLM。  
  - 示例：`北京天气`、`从厦门到上海的车票`、`3分钟后提醒我喝水`、`贵州茅台股价`、`喜加一`、`今日运势`、`帮我查一下 xxx`、`搜索 关键词`、`支出 20 咖啡`、`今天花了多少` 等。  
  - 配置：**natural_language** 中可关闭总开关 **「启用自然语言触发」**（`nl_enabled`）以完全关闭自然语言，或按模块关闭（如 `nl_weather_enabled`）。关闭后仅响应 `/` 开头的指令。**若在配置里关闭后仍看到「[NL] 开始匹配」日志，请保存配置后重启机器人或重新加载插件，使配置生效。**  
...
- **结构**：按功能模块分组，每个模块一个顶层对象（与项目根目录「样例」写法一致）：
  ...
  - `natural_language`：自然语言触发总开关 `nl_enabled` 及各模块开关（`nl_weather_enabled`、`nl_train_enabled` 等）
- 配置项仍使用**模块前缀**命名（如 `train_*`、`sy_*`、`weather_*`、`nl_*` 等），避免冲突。
...
- **2026-03-01**：新增 **百度千帆智能搜索 / 网页搜索**：命令 `/智能搜索 <问题>`（别名 `/智能搜素`）、`/搜索 <关键词>`；鉴权为 **API Key**（请求头 `X-Appbuilder-Authorization: Bearer <API Key>`）；配置项 `qianfan_search_ak`；**本地统计**每日次数，`/智能搜索` 达 100 次、`/搜索` 达 1000 次后拒绝调用；`/搜索` 的结果会交给当前 LLM 整理后输出。
